-- Exposes applications.status_changed_at on v_allotment_board — needed by
-- the Dashboard's IPO progress cards to drop an IPO out of view for a
-- viewer once their own application(s) on it came back NOT_ALLOTTED more
-- than a day ago, instead of leaving it sitting there until the IPO's
-- listing date (isLiveIpo's window) or full archival. Row-level RLS on
-- applications already scopes boardRows to "this viewer's own rows" for a
-- funder-only viewer and "everyone's" for admin, so the same "all of my
-- visible rows for this IPO are stale NOT_ALLOTTED" check works correctly
-- for both without any role branching in the client.

-- Exposes the IPO's close_date on v_allotment_board — needed by the
-- Dashboard's "Awaiting mandate approval" tile to tell apart mandates that
-- can still actually be approved (before 4:50pm IST on the close date, the
-- same bidding cutoff isOpenForBidding/hasBiddingClosed already enforce
-- elsewhere) from ones sitting PENDING past that point, where approving no
